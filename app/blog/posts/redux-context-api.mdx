...
<Image width="800" height="800" src="/blog/redux-context-api/redux-flow.png" alt="redux-flow" />

Listed below are the components that makeup Redux:

- <u>State</u>: A state is where all the logic of your application lies. Basically, the state
  describes how your application works, and the UI is rendered based on the state.
- <u>Actions</u>: Actions are methods that are dispatched to trigger an event. Simply, an action is
  a way to notify that an event has occurred within the application.
- <u>Reducers</u>: Reducers are methods that catch different actions triggered and modifies the
  logic to change and update the state of the application.
- <u>Subscriptions</u>: Subscriptions are methods to make use of the state in your React application
  (by the components).
...
<Callout emoji="💡">
  Prop Drilling is the process by which you pass data from one part of the React Component tree to
  another by going through other parts that do not need the data but only help in passing it around.
</Callout>

<Image width="800" height="800" src="/blog/redux-context-api/context-api.png" alt="context-api" />
...
<u>Note</u>: The context can be made available to all the components of the application or to only a
few components down the tree. For the former case, we can pass the context provider in the root
component. And for the latter, we can provide it to any particular component down the component
tree.
